...
/**
 * it is easy to use , for exemple:
 *
 * var resultList = [{id:30,isOpen:true},{id:50,isOpen:true},{id:70,isOpen:true}];
 * 
 * update(resultList).set('id',99);
 * update(resultList).set('isTrue','!isTrue'); // invert value
 * update(resultList).set('id',' * = 300'); // expression
 * //TODO : implements where 
 */ 
function update(resultList)
{
	return {set:function(prop,expression)
		{
			var resultString = 'resultList[i]';
			
			if((/!/).test(expression))
			{
				expression = expression.replace('!','');
				expression = '!'+resultString +'.'+ expression;
			}
			...
			if(!((/[=,>,<,\+,-,\*,\/]/).test(expression)))
				...
				{
					prop = resultString +'.'+ prop;
				}
			for ( var i = 0; i < resultList.length; i++) 
			{
				var sub = resultString+"."+prop+" "+expression; 
				eval(sub);
			}
			return resultList;
		}};
